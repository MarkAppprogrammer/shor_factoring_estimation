...
if __name__ == '__main__':
    ...
    for n in num_bits:
        params = Params('3dcolor',
                        AlgoOpts(n=n, ne=ne_size(n), windowed=True),
                        LowLevelOpts())
        ...
        #print(f"n: {n} Time: {format_time(cost.exp_t, unicode=True)} Qubits: {qubits}")
        print(f"{n}, {float(format_time(cost.exp_t, unicode=True).strip(' days')) * (qubits / (10 ** 6))}, {(qubits / (10 ** 6))}, {float(format_time(cost.exp_t, unicode=True).strip(' days'))} ")

    """Only edits made here was commenting out the code below"""
